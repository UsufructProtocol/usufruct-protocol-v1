...
The policy that configures how price scales with demand across successive tenancies. Instead of the owner setting an explicit price, the market sets it: each tenant's committed stake becomes the reference point for the next tenant's minimum floor. The protocol reads the reference stake from the most recent bid — in `Occupied` state this is the current tenant's stake; in `Demand` state it is the pending (latest) bid's stake, reflecting the most recent signal of market willingness to pay. `FixedDelta` adds a constant increment on top of that reference, creating a simple linear ratchet. `CompoundDelta` applies a percentage growth plus a fixed minimum increment, enabling compounding trajectories for high-demand assets where each successive tenant signals stronger conviction.

Its counterpart is `AuctionWindowPolicy` with `auction_shape`: after a tenure expires without a waiting tenant, the escalated price could otherwise sit frozen at a level the market can no longer reach. The auction descent window is what corrects this — it brings the price back down to the rest floor over a configurable time and at a rate shaped by `auction_shape`. Together, escalation and descent form the protocol's full price discovery loop: up with demand, down when demand stalls.
...
- `price_escalation_policy::compute_next_price(&PriceEscalationPolicy, price: Price): Price` — applies the escalation formula to `price` (the previous tenant's stake reinterpreted as a reference price) and returns the escalated floor.
