...
```
Resolved policy parameters for the current cycle. Computed once when the cycle begins by resolving every policy to its domain primitive (the policies are deterministic — no randomness); carried through the cycle so the same values apply consistently.
...
```
OccupiedTerms<CoinType> { schedule: TenancySchedule, active: UsufructuarySeat<CoinType>, retire: RetireCondition }
```
...
**Rental**
- `asset_state::execute_rent<Asset, C>(AssetState, EscrowCore<C>, payment: Coin<C>, tenures: Tenures, clock, ctx): (RentingState<Asset, C>, EscrowCore<C>, UsufructCap)` — advances state machine first, validates `TenureExtendPolicy`, then:
...
**Governor operations**
- `asset_state::execute_retire<Asset, C>(&mut AssetState, &mut EscrowCore<C>, governance_cap, clock, ctx)` — validates commitment has elapsed; advances state machine first; then: transitions immediately to `Retired` from `Idle` or `Descent`; sets the `RetireCondition` flag from `Occupied` or `Demand` so the next tenure expiry goes to `Retired` instead of `Descent`.
- `asset_state::execute_update_config<Asset, C>(AssetState, EscrowCore<C>, governance_cap, new_ensemble, clock, ctx): (AssetState, EscrowCore<C>)` — advances state machine first, then:
  - `Idle` → applies immediately, re-resolves `CycleParams` from new ensemble; emits `EnsembleUpdated`.
  - `Descent`, `Occupied`, `Demand` → stages as pending; emits `EnsembleUpdateScheduled`. Aborts if retire flag is already set.
  - `Retired` → aborts.
...
- `asset_state::execute_claim<Asset, C>(AssetState, EscrowCore<C>, governance_cap: &GovernanceCap, clock, ctx): Asset` — advances state machine first; then consumes state and core and unlocks the asset, returning **only the asset**. Governor income was settled to the `EarningsInbox` each settlement (never accumulated in the seat), so there is nothing to sweep. Takes `&GovernanceCap` — not consumed, since the cap may govern other escrows. Aborts unless state is `Retired` after the advance.

**Cap management**
- `asset_state::execute_burn_stale_usufruct_cap<Asset, C>(&AssetState, &EscrowCore<C>, cap: UsufructCap, clock, ctx)` — burns a stale cap; asserts it is neither current nor pending.
...
- **`Descent → Idle`** (`step_auction_expiry` → `do_auction_expiry`): auction window elapsed with no bid; applies pending ensemble if present (emits `EnsembleUpdated`); re-resolves a fresh `CycleParams` from the now-active ensemble; emits `AuctionExpired`. This is the only transition that applies a staged config update.
...
```
RentStarted {
    escrow_id: ID, usufruct_cap_id: ID, usufructuary_address: address,
...
BidPlaced {
    escrow_id: ID,
    active_usufruct_cap_id: ID, active_usufructuary_address: address,
    active_stake_balance: u64, active_phase_start_ms: u64,
    pending_usufruct_cap_id: ID, pending_usufructuary_address: address,
    bid_amount: u64, floor_price: u64,
    handover_countdown_expiry: u64, committed_tenures: u64,
...
BidSuperseded {
    ...
    active_usufruct_cap_id: ID, active_usufructuary_address: address,
    active_stake_balance: u64, active_phase_start_ms: u64,
    displaced_usufruct_cap_id: ID, displaced_bidder_address: address, refunded_amount: u64,
    pending_usufruct_cap_id: ID, pending_bidder_address: address, pending_bid_amount: u64,
    floor_price: u64, handover_countdown_expiry: u64, committed_tenures: u64,
    asset_type: String, coin_type: String, timestamp_ms: u64
}
```
Emitted when a higher bid supersedes the standing pending bid via `do_supersede_bid`. Three parties: **active** (the untouched incumbent), **displaced** (the prior pending bid, refunded), **pending** (the new leading bid).

```
HandoverCompleted {
    escrow_id: ID,
    departing_usufruct_cap_id: ID, departing_usufructuary_address: address,
...
```
Emitted when the handover countdown fires and the pending bid becomes the active occupant. The **departing** bundle (`departing_phase_start_ms`, `departing_ceiling_total_ms`, `departing_handover_total_ms`) gives the full temporal context of the outgoing tenancy — including the original planned ceiling — so consumers can compute the shortfall without a look-back join. The incoming schedule fields (`committed_tenures`, `ceiling_total_ms`, `handover_total_ms`) describe the new active usufructuary's terms, rescaled from the departing usufructuary's schedule by the incoming/outgoing tenure ratio.

```
TenureExpired {
    escrow_id: ID, usufruct_cap_id: ID, usufructuary_address: address,
    phase_start_ms: u64, governor_share: u64, protocol_fee: u64,
    last_acquisition_price: u64, asset_type: String, coin_type: String, timestamp_ms: u64
}
```
...
AuctionExpired {
    ...
    last_acq_price: u64, asset_type: String, coin_type: String, timestamp_ms: u64
}
```
...
```
AssetRetired { escrow_id: ID, asset_type: String, coin_type: String, timestamp_ms: u64 }
```
...
```
AssetBorrowed { escrow_id: ID, usufruct_cap_id: ID, usufructuary_address: address, asset_type: String, coin_type: String, timestamp_ms: u64 }
```
Emitted when a usufructuary extracts the asset via `execute_borrow`.

```
AssetReturned { escrow_id: ID, usufruct_cap_id: ID, usufructuary_address: address, asset_type: String, coin_type: String }
```
...
> Config-update events (`EnsembleUpdated`, `EnsembleUpdateScheduled`) and the registration snapshot (`PolicyEnsembleRegistered`) are **emitted by `policy_ensemble`**, not defined here — `execute_update_config` delegates to `policy_ensemble::emit_*`. See `policies/policy_ensemble`.

```
RetireCommitmentExtended {
    escrow_id: ID, commitment_policy: String, commitment_floor_ms: Option<u64>,
    new_unlock_at_ms: u64, asset_type: String, coin_type: String, timestamp_ms: u64
}
...
```
Emitted on `execute_extend_commitment` for the retire-commitment and ensemble-commitment locks respectively — each carries the new unlock time and the policy that produced it.

```
RetireFlagSet { escrow_id: ID, governance_cap_id: ID, governor_address: address, asset_type: String, coin_type: String, timestamp_ms: u64 }
```
