 ...
 *
 * Measures: earnings::collect_earnings_messages<SUI> with N tickets — the owner-
 ...
 *   so all N escrows route their owner-share income to the SAME EarningsInbox.
 ...
 *
 *   The inbox is owned by the profiling owner, so collection signs as the owner —
...
async function applyPending(
  ...
  const r = await client.signAndExecuteTransaction({ transaction: tx, signer: owner, options: { showEffects: true } });
...
async function generateEarningsMessages(
  ...
  for (let i = 0; i < n; i++) {
    ...
    tx.setSender(d.owner.address);
    if (i === 0) {
      const { ownerCap, inbox } = buildIntegrate(tx, d.usufructPackageId, d.dummyAssetPackageId, d.protocolFeeRefId, build1msEnsemble);
      tx.transferObjects([ownerCap, inbox], d.owner.address);
    } else {
      buildIntegrateIntoPortfolio(tx, d.usufructPackageId, d.dummyAssetPackageId, d.protocolFeeRefId, ownerCapId, inboxId, build1msEnsemble);
    }
  ...
  for (let i = 0; i < n; i++) {
    ...
    const r = await client.signAndExecuteTransaction({ transaction: tx, signer: kp.tenant1, options: { showEffects: true } });
    await client.waitForTransaction({ digest: r.digest });
    if ((i + 1) % 10 === 0) process.stdout.write(`${i + 1}`); else process.stdout.write('.');
  }
  console.log(` ✓`);
  ...
  for (let i = 0; i < n; i++) {
    let ok = false;
    for (let attempt = 0; attempt < 3 && !ok; attempt++) ok = await applyPending(client, kp.owner, d, escrows[i]);
