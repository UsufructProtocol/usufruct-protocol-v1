 ...
 * Flow: integrate → rent → [wait tenure] → retire → apply → claim_asset
 * Uses 2s tenure so the lifecycle completes without waiting 1h.
 * Gas costs are tenure-duration-independent.
 */
...
async function main() {
  ...
  const out = steps.map(s => ({
    ...s,
    computation: s.computation.toString(), storage: s.storage.toString(),
    rebate: s.rebate.toString(), nonRefundable: s.nonRefundable.toString(),
    net: s.net.toString(),
  }));
