...
One of the two governor-side trust commitments. It governs **stability of terms**: whether the governor is permitted to call `update_ensemble` to change the escrow's `PolicyEnsemble` (rest price, tenure duration, curves, handover/auction windows, price escalation). It is set at integration time. An `Immediate` policy offers no guarantee — the governor can change the terms at any moment (subject to the existing per-state scheduling rules of `update_ensemble`). A `Deferred` policy **freezes the entire ensemble** until at least `floor` milliseconds have elapsed since the commitment anchor, giving usufructuaries and prospective usufructuaries a contractual guarantee that the rules will not move out from under them. The governor can only extend this commitment, never shorten it.
