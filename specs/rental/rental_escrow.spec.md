...
- Emit `BidPlaced { escrow_id, tenant_cap_id, pending_tenant, bid_amount,
  ...
  pending, so the event's PK fields all reference live data.
- Return `cap`.
...
- Emit `BidSuperseded { escrow_id, displaced_tenant_cap_id,
  ...
  new_bid_amount, floor_price: floor }` — emit-last: all state rotations,
  the refund push, and the new cap construction complete; both cap PKs
  refer to live (or just-staled) data.
- Return `cap`.
...
4. **Settle `used_credit`** — §7.6 is the single source of truth for
   ...
   - `let (owner_share, protocol_fee) = settle_stake_earnings(escrow, used_credit, displaced_tenant, ctx);`
     ...
     consumed by the `HandoverCompleted` emit at step 8.
...
8. Emit `HandoverCompleted { escrow_id, displaced_tenant,
   ...
   `TenantCapMinted.tenant` row (emitted at bid time, not here — under
   eager minting `do_handover` does not mint) and recoverable by JOIN
   on `new_tenant_cap_id`. `displaced_tenant` *is* carried: no PK path
...
| **[new] Q11** | `compute_floor_price` after a settle with `descent_ceiling = 0`. Setup: install a tenant with a config where `descent_ceiling = 0`; advance the clock past `tenure_ceiling`; call `apply_pending_transitions`. | Post-settle: `escrow.state == Idle` (NOT `AtDutchAuction`). `compute_floor_price(escrow, clock.now())` returns `config.min_rent_price` (the Idle dispatch branch), **not** `compute_price_descent`. Co-emitted events: `TenureExpired` and `AuctionExpired` with identical `timestamp_ms == boundary_ms`. Asserts the structural invariant that AtDutchAuction is unobservable post-settle under `descent_ceiling = 0` — Check 3 always chains in the same `apply_pending_transitions` call as Check 2 because `phase_start_ms + 0 ≤ clock.now()` is trivially true the moment Check 2 sets `phase_start_ms = boundary_ms`. The `compute_floor_price` `match` therefore never reaches the `AtDutchAuction` arm under this configuration. |
...
| **[new] M6b** | Rented(HandoverOpen), config has `descent_ceiling = 0` | tenure expired (descent expiry coincides with tenure expiry by definition) | C2 → C3 (atomic, same APT call) | Idle | Idle | Same shape as M6 but reached via the `descent_ceiling = 0` path — the "pure machine — no price memory" variant. The escrow never observably enters `AtDutchAuction`: Check 3 fires in the same APT call as Check 2 because `phase_start_ms + 0 ≤ clock.now()` is trivially true once C2 has just set `phase_start_ms = boundary_ms`. `TenureExpired` and `AuctionExpired` co-emit with identical `timestamp_ms == boundary_ms`, then `RentStarted(from_state: Idle)`. Pairs with Q11 — Q11 covers the read path (`compute_floor_price` dispatch never reaches the AtDutchAuction arm), this row covers the write path (state machine never leaves the escrow in AtDutchAuction). |
