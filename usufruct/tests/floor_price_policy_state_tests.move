...
#[test_only]
module usufruct::floor_price_policy_state_tests;

use std::unit_test::assert_eq;
use usufruct::floor_price_policy_state;
...
fun projectors_fixed_variant() {
    let p = floor_price_policy_state::new_fixed(monetary::price(100));
...
fun projectors_random_in_range_variant() {
    let p = floor_price_policy_state::new_random_in_range(monetary::price(50), monetary::price(200));
    assert!(!p.proj_is_fixed());
    assert!(p.proj_is_random_in_range());
