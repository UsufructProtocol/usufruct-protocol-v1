...
fun stake_value_of_reads_inner_balance() {
    let t = t1();
    let s = tenant_seat::proj_stake(&t);
    assert_eq!(tenant_stake::proj_value(s), monetary::stake(STAKE_T1));
    tenant_seat::destroy_for_testing(t);
}
...
fun take_fee_share_partial_reduces_stake_and_returns_typed_share() {
    let mut t = t1();
    let share = tenant_seat::take_fee_share(&mut t, monetary::stake(75), escrow_identity::new(fake_escrow_id()));
    ...
    assert_eq!(tenant_seat::proj_stake_value(&t), monetary::stake(STAKE_T1 - 75));
...
fun take_fee_share_zero_returns_zero_share_unchanged_stake() {
    let mut t = t1();
    let share = tenant_seat::take_fee_share(&mut t, monetary::stake(0), escrow_identity::new(fake_escrow_id()));
...
fun take_fee_share_more_than_stake_aborts() {
    let mut t = t1();
    let share = tenant_seat::take_fee_share(&mut t, monetary::stake(STAKE_T1 + 1), escrow_identity::new(fake_escrow_id()));
    fee_message::destroy_share_for_testing(share);
    tenant_seat::destroy_for_testing(t);
}
...
fun liquidate_sends_full_stake_to_address_as_coin() {
    ...
    sc.next_tx(@0xCAFE);
    {
        let (_id, stake) = tenant_seat::unbundle(t1());
