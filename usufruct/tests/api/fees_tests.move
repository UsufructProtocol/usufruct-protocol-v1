...
const USUFRUCTUARY:  address = @0xA1;
...
fun mk_usufructuary_seat(stake: u64): usufructuary_seat::UsufructuarySeat<SUI> {
    usufructuary_seat::new(
        usufruct_cap::from_id(object::id_from_address(@0xCA1)),
        refund_address::new(USUFRUCTUARY),
...
fun fi1_inbox_id_returns_inbox_id() {
    ...
    sc.next_tx(GOVERNOR);
    {
        ...
        assert_eq!(fees::inbox_id(&fee_ref), inbox_id);
...
fun fc1_collect_empty_vector_returns_zero_coin() {
    ...
    sc.next_tx(GOVERNOR);
    {
        ...
        let coin      = fees::collect_fee_messages<SUI>(&mut inbox, vector[], sc.ctx());
...
// Flow: integrate → drive_to_rented (inject usufructuary stake) →
//       fire_do_tenure_expiry (posts FeeMessage) → collect via fees API.
#[test]
fun fc2_collect_protocol_fees_from_real_tenure_expiry() {
    ...
    // --- Block 2: inject usufructuary and drive to rented, then expire the tenure ---
    let mut msg_id: ID;
    sc.next_tx(GOVERNOR);
    {
    ...
    sc.next_tx(GOVERNOR);
    {
        ...
        let coin      = fees::collect_fee_messages<SUI>(&mut inbox, vector[ticket], sc.ctx());
