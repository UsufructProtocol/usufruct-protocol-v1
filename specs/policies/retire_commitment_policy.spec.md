...
One of the two governor-side trust commitments. It governs **availability**: when the governor is permitted to retire the escrow and reclaim the asset. It is set at integration time. An `Immediate` policy offers no guarantee — the governor can exit as soon as the escrow is idle, which usufructuaries will price into their willingness to commit stake. A `Deferred` policy locks the governor out of retirement until at least `floor` milliseconds have elapsed since the commitment anchor, giving usufructuaries a contractual guarantee of minimum availability. The governor can only extend this commitment, never shorten it, making deferred commitments a credible and verifiable signal on-chain. Assets that benefit most are those whose value to usufructuaries depends on continuity — a domain name, a recurring-use access key, a long-horizon position — where usufructuaries need assurance the asset will not be pulled mid-market.

Its twin is [[ensemble_commitment_policy]], which commits the governor to **stability of terms** rather than availability. Permanence without price stability is a hollow promise, so the two together form one credibility story: this policy promises the asset stays in the market, the twin promises the rules of the market stay fixed.
...
- `Immediate` — no lockup; the governor can retire as soon as the escrow is idle.
