...
async function main() {
  ...
  const cap1Id = (c2.find(c => c.type === 'created' && (c as any).objectType?.includes('UsufructCap')) as any).objectId;

  // 3. usufructuary1 redirects refund — the feature step.
  ...
  tx3.moveCall({
    target:        `${d.usufructPackageId}::escrow::update_usufructuary_refund_address`,
    typeArguments: typeArgs,
    arguments:     [tx3.object(escrowId), tx3.object(cap1Id), refund, clock(tx3)],
  });
  const r3 = await measure(client, kp.usufructuary1, 'update_refund_address', 0, tx3);
  steps.push(r3);
  console.log(` net=${r3.net}`);

  // 4. usufructuary2 bids → Demand
  process.stdout.write('Step 4 rent(t2)...');
  const tx4 = new Transaction();
  tx4.setSender(d.usufructuary2.address);
  ...
  tx4.transferObjects([cap2], d.usufructuary2.address);
  const r4 = await measure(client, kp.usufructuary2, 'rent_t2', 0, tx4);
  steps.push(r4);
  ...
  const r5 = await measure(client, kp.governor, 'apply_handover', 0, tx5);
  steps.push(r5);
  ...
  tx7.setSender(d.governor.address);
  tx7.moveCall({
    target:        `${d.usufructPackageId}::escrow::retire`,
    typeArguments: typeArgs,
    arguments:     [tx7.object(escrowId), tx7.object(governanceCapId), clock(tx7)],
  });
  const r7 = await measure(client, kp.governor, 'retire', 0, tx7);
  steps.push(r7);
  ...
  tx8.setSender(d.governor.address);
  tx8.moveCall({
    target:        `${d.usufructPackageId}::escrow::apply_pending_transition_states`,
    typeArguments: typeArgs,
    arguments:     [tx8.object(escrowId), clock(tx8)],
  });
  const r8 = await measure(client, kp.governor, 'apply_transitions', 0, tx8);
  steps.push(r8);

  // 9. claim.
  process.stdout.write('Step 9 claim...');
  const tx9 = new Transaction();
  tx9.setSender(d.governor.address);
  const asset = tx9.moveCall({
    target:        `${d.usufructPackageId}::escrow::claim_asset`,
    typeArguments: typeArgs,
    arguments:     [tx9.object(escrowId), tx9.object(governanceCapId), clock(tx9)],
  });
  tx9.transferObjects([asset], d.governor.address);
  const r9 = await measure(client, kp.governor, 'claim_asset', 0, tx9);
  steps.push(r9);
