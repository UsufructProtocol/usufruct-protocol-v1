...
- **`IntegrationConfig`.** Not constructed inline per row. Every test
  draws from a fixed **deterministic corpus** of 168 configs defined in
  `specs/rental/escrow_corpus.spec.md`. Each row of §10.1–§10.13
  specifies which projection of the corpus it iterates over; the
  materialized module is `usufruct::rental_escrow_corpus`
  (`usufruct/tests/rental_escrow_corpus.move`).
...
The corpus design — axes, cardinality, tag scheme τ2, module API,
named projections, operational rules, and self-integrity tests — is
fully specified in **`specs/rental/escrow_corpus.spec.md`**. The
materialized module is `usufruct::rental_escrow_corpus`
(`usufruct/tests/rental_escrow_corpus.move`). All cross-references
in §10.1–§10.13 to projections such as `with_descent_skipped()`,
`filter_c(...)`, or `by_tag(...)` refer to that module's API as
documented in that companion spec.
