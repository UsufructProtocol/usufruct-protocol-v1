...
- **Publisher authority is package-scoped, not module-scoped.**
  ...
  `ProtocolFeeInbox`, `PaymentReceipt`, and any future type that needs
...
| I2 | On the taken `Publisher`, assert `sui::package::from_package<T>(&publisher)` for every protocol type | Returns `true` for `OwnerCap`, `TenantCap`, `ProtocolFeeInbox`, `ProtocolFeeRef`, `PaymentReceipt`, and `RentalEscrow<A, C>` (any instantiation). Asserts single-Publisher authority covers the full package surface. |
...
| P2 | Repeat P1 for each of the other three types (`TenantCap`, `ProtocolFeeInbox`, `PaymentReceipt`) | All four registrations succeed with the same `Publisher`. Confirms P2 (package-scoped authority) empirically. |
