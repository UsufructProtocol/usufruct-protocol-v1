...
#[test_only]
module usufruct::tenure_policy_state_tests;

use std::unit_test::assert_eq;
use usufruct::tenure_policy_state;
use usufruct::phases;
...
fun projectors_fixed_variant() {
    let p = tenure_policy_state::new_fixed(phases::duration(100));
...
fun projectors_random_in_range_variant() {
    let p = tenure_policy_state::new_random_in_range(phases::duration(50), phases::duration(200));
    assert!(!p.proj_is_fixed());
    assert!(p.proj_is_random_in_range());
