...
// R2: the wrapper output is accepted by escrow::update_usufructuary_refund_address —
...
#[test]
fun r2_refund_address_feeds_update_usufructuary_refund_address() {
    let mut sc = setup();
    sc.next_tx(GOVERNOR);
    ...
    escrow::update_usufructuary_refund_address(
        &mut escrow, &usufruct_cap, refund::refund_address(NEW_REFUND), &clk2, sc.ctx(),
    );

    // The seat now reflects the redirected address.
    assert_eq!(*escrow::active_usufructuary_addr(&escrow).borrow(), NEW_REFUND);
...
fun r3_refund_address_feeds_update_on_pending_seat() {
    ...
    let active_before = *escrow::active_usufructuary_addr(&escrow).borrow();

    escrow::update_usufructuary_refund_address(
        &mut escrow, &cap_t2, refund::refund_address(NEW_REFUND), &clk2, sc.ctx(),
    );

    assert_eq!(*escrow::pending_usufructuary_addr(&escrow).borrow(), NEW_REFUND);
    assert_eq!(*escrow::active_usufructuary_addr(&escrow).borrow(),  active_before);

    transfer::public_transfer(cap_t1, USUFRUCTUARY_ADDR);
    transfer::public_transfer(cap_t2, USUFRUCTUARY_ADDR);
    test_scenario::return_shared(escrow);
    clock::destroy_for_testing(clk2);
    transfer::public_transfer(governance_cap, GOVERNOR);
    sc.end();
