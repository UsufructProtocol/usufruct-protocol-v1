#!/usr/bin/env tsx
/**
 * Phase B / 03 — Usufructuary rotation via handover
...
async function main() {
  ...
  // 3. usufructuary2 bids (→ Demand, cap1 becomes stale)
  ...
  const rApp = await measure(client, kp.governor, 'apply_handover', 0, txApp);
  steps.push(rApp);
  ...
  tx5.moveCall({
    ...
    typeArguments: typeArgs,
    arguments: [tx5.object(escrowId), tx5.object(governanceCapId), clock(tx5)],
  });
  const r5 = await measure(client, kp.governor, 'retire', 0, tx5);
  steps.push(r5);
  ...
  const rApp2 = await measure(client, kp.governor, 'apply_transitions', 0, txApp2);
  steps.push(rApp2);
