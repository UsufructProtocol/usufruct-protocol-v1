 ...
 *           EXISTING cap+inbox pair). Mints NO inbox (+2 obj: Escrow + TenantSeat-
 ...
 *           into a fleet governed by one OwnerCap and one EarningsInbox.
...
async function setupPair(
...
): Promise<{ ownerCapId: string; inboxId: string }> {
  ...
  const ownerCapId = (changes.find(c => c.type === 'created' && (c as any).objectType?.includes('OwnerCap')) as any).objectId;
  const inboxId    = (changes.find(c => c.type === 'created' && (c as any).objectType?.includes('EarningsInbox')) as any).objectId;
  return { ownerCapId, inboxId };
}
...
async function main() {
  ...
  const { ownerCapId, inboxId } = await setupPair(client, kp.owner, d);
  console.log(` cap=${ownerCapId.slice(0, 10)}…  inbox=${inboxId.slice(0, 10)}…`);
  ...
  for (let run = 0; run < RUNS; run++) {
    ...
    const rec = await measure(client, kp.owner, 'integrate_into_portfolio', run, tx);
    records.push(rec);
