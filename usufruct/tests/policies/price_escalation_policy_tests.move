...
#[test_only]
module usufruct::price_escalation_policy_tests;

use std::unit_test::assert_eq;
use usufruct::{math, monetary, price_escalation_policy};
...
fun new_fixed_delta_success() {
    ...
    cases.do_ref!(|case| {
        let pf = price_escalation_policy::new_fixed_delta(monetary::price(case.delta));
        assert_eq!(price_escalation_policy::fixed_delta_fields_for_testing(&pf), case.delta);
    });
}
...
fun new_compound_delta_success() {
    let bpu = price_escalation_policy::bps_per_unit_for_testing();
    let cases = vector[
    ...
    cases.do_ref!(|case| {
        let pf = price_escalation_policy::new_compound_delta(math::bps(case.bps), monetary::price(case.delta));
        let (stored_bps, stored_delta) = price_escalation_policy::compound_delta_fields_for_testing(&pf);
...
#[test, expected_failure(abort_code = price_escalation_policy::EDeltaZero, location = usufruct::price_escalation_policy)]
fun new_fixed_delta_delta_zero_aborts() {
    price_escalation_policy::new_fixed_delta(monetary::price(0));
}

#[test, expected_failure(abort_code = price_escalation_policy::EDeltaZero, location = usufruct::price_escalation_policy)]
fun new_compound_delta_delta_zero_aborts() {
    price_escalation_policy::new_compound_delta(math::bps(500), monetary::price(0));
}

#[test, expected_failure(abort_code = price_escalation_policy::EBpsRange, location = usufruct::price_escalation_policy)]
fun new_compound_delta_bps_zero_aborts() {
    price_escalation_policy::new_compound_delta(math::bps(0), monetary::price(1));
}
...
fun new_compound_delta_bps_one_above_upper_bound_aborts() {
    ...
    price_escalation_policy::new_compound_delta(math::bps(18_446_744_073_709_541_616), monetary::price(1));
}

#[test, expected_failure(abort_code = price_escalation_policy::EBpsRange, location = usufruct::price_escalation_policy)]
fun new_compound_delta_bps_max_aborts() {
    price_escalation_policy::new_compound_delta(math::bps(18_446_744_073_709_551_615), monetary::price(1)); // u64::MAX saturated
}
...
fun eval_fixed_delta_golden_vectors_and_strict_increase() {
    ...
    cases.do_ref!(|case| {
        let r = price_escalation_policy::eval_fixed_delta_for_testing(case.price, case.delta);
...
fun eval_fixed_delta_overflow_max_plus_one_aborts() {
    price_escalation_policy::eval_fixed_delta_for_testing(18_446_744_073_709_551_615, 1); // u64::MAX + 1
}

#[test, expected_failure(abort_code = monetary::EPriceAddOverflow, location = usufruct::monetary)]
fun eval_fixed_delta_overflow_half_each_aborts() {
    // (u64::MAX/2 + 1) + (u64::MAX/2 + 1) = u64::MAX + 1
    price_escalation_policy::eval_fixed_delta_for_testing(9_223_372_036_854_775_808, 9_223_372_036_854_775_808);
}
...
fun eval_compound_delta_golden_vectors() {
    ...
    cases.do_ref!(|case| {
        let r = price_escalation_policy::eval_compound_delta_for_testing(
...
#[test, expected_failure(arithmetic_error, location = usufruct::price_escalation_policy)]
fun eval_compound_delta_overflow_add_delta_aborts() {
    ...
    price_escalation_policy::eval_compound_delta_for_testing(9_223_372_036_854_775_807, 1, 18_446_744_073_709_551_615);
}
...
fun eval_compound_delta_pct_floor_threshold_seed_set_c() {
    ...
    cases.do_ref!(|case| {
        let below = price_escalation_policy::eval_compound_delta_for_testing(case.threshold - 1, case.bps, 1);
        assert_eq!(below, case.threshold); // (threshold-1) + delta=1 = threshold; pct = 0
        let at = price_escalation_policy::eval_compound_delta_for_testing(case.threshold, case.bps, 1);
...
fun eval_fixed_delta_composition() {
    ...
    while (i < len) {
        x = price_escalation_policy::eval_fixed_delta_for_testing(x, delta);
...
fun eval_compound_delta_composition_100pct() {
    ...
    let len = expected.length();
    while (i < len) {
        x = price_escalation_policy::eval_compound_delta_for_testing(x, bps, delta);
        assert_eq!(x, expected[i]);
        i = i + 1;
    };
}
...
fun compound_equals_fixed_below_pct_floor_threshold() {
    ...
    while (i < len) {
        ...
        let compound = price_escalation_policy::eval_compound_delta_for_testing(x, bpss[i], delta);
        let fixed    = price_escalation_policy::eval_fixed_delta_for_testing(x, delta);
...
fun eval_compound_delta_monotone_in_bps() {
    ...
    while (i < len) {
        let lo = price_escalation_policy::eval_compound_delta_for_testing(x, bps_chain[i - 1], delta);
        let hi = price_escalation_policy::eval_compound_delta_for_testing(x, bps_chain[i],     delta);
        assert!(lo < hi, 0);
        i = i + 1;
    };
}
...
fun eval_compound_delta_monotone_in_price() {
    ...
    let len = prices.length();
    while (i < len) {
        let lo = price_escalation_policy::eval_compound_delta_for_testing(prices[i - 1], bps, delta);
        let hi = price_escalation_policy::eval_compound_delta_for_testing(prices[i],     bps, delta);
        assert!(lo <= hi, 0);
        i = i + 1;
    };
}
...
fun projectors_fixed_delta_variant() {
    let p = price_escalation_policy::new_fixed_delta(monetary::price(100));
...
fun projectors_compound_delta_variant() {
    let p = price_escalation_policy::new_compound_delta(math::bps(250), monetary::price(50));
    assert!(!p.proj_is_fixed_delta());
    assert!(p.proj_is_compound_delta());
