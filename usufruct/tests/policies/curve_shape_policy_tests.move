...
#[test_only]
module usufruct::curve_shape_policy_tests;

use std::unit_test::assert_eq;
use usufruct::curve_shape_policy;
use usufruct::phases;
...
fun constructors_no_fields_succeed() {
    let _ = curve_shape_policy::new_linear();
    let _ = curve_shape_policy::new_smoothstep();
    let _ = curve_shape_policy::new_logistic();
}
...
fun new_power_law_reduces_via_gcd_and_stays_coprime() {
    ...
    cases.do_ref!(|case| {
        let shape = curve_shape_policy::new_power_law(case.in_num, case.in_den);
        let (n, d) = curve_shape_policy::power_law_fields_for_testing(&shape);
...
fun new_exponential_lower_and_upper_bounds_succeed() {
    let _ = curve_shape_policy::new_exponential(1, false); // lower bound, convex
    let _ = curve_shape_policy::new_exponential(8, true);  // upper bound, concave
}

// ─── Constructors — abort ──────────────────────────────────────────────────

#[test, expected_failure(abort_code = curve_shape_policy::EAlphaNumRange, location = usufruct::curve_shape_policy)]
fun new_power_law_alpha_num_zero_aborts() {
    let _ = curve_shape_policy::new_power_law(0, 2);
}

#[test, expected_failure(abort_code = curve_shape_policy::EAlphaNumRange, location = usufruct::curve_shape_policy)]
fun new_power_law_alpha_num_above_8_aborts() {
    let _ = curve_shape_policy::new_power_law(9, 2);
}

#[test, expected_failure(abort_code = curve_shape_policy::EAlphaDenRange, location = usufruct::curve_shape_policy)]
fun new_power_law_alpha_den_zero_aborts() {
    let _ = curve_shape_policy::new_power_law(3, 0);
}

#[test, expected_failure(abort_code = curve_shape_policy::EAlphaDenRange, location = usufruct::curve_shape_policy)]
fun new_power_law_alpha_den_above_4_aborts() {
    let _ = curve_shape_policy::new_power_law(3, 5);
}

#[test, expected_failure(abort_code = curve_shape_policy::EDegenerateLinear, location = usufruct::curve_shape_policy)]
fun new_power_law_degenerate_2_2_aborts() {
    let _ = curve_shape_policy::new_power_law(2, 2);
}

#[test, expected_failure(abort_code = curve_shape_policy::EDegenerateLinear, location = usufruct::curve_shape_policy)]
fun new_power_law_degenerate_4_4_aborts() {
    let _ = curve_shape_policy::new_power_law(4, 4);
}

#[test, expected_failure(abort_code = curve_shape_policy::EAlphaAbsRange, location = usufruct::curve_shape_policy)]
fun new_exponential_alpha_abs_zero_aborts() {
    let _ = curve_shape_policy::new_exponential(0, false);
}
...
fun evaluate_curve_edge_cases_apply_to_every_variant() {
    let scale = curve_shape_policy::scale_for_testing();
    let cases = vector[
    ...
    let seeds: vector<curve_shape_policy::CurveShapePolicy> = vector[
    ...
    while (s < slen) {
        ...
        cases.do_ref!(|case| {
            assert_eq!(
                curve_shape_policy::proj_value(curve_shape_policy::compute_curve_height(shape, curve_shape_policy::progress(phases::elapsed_for_testing(case.t), phases::duration(case.t_max)))),
                case.expected,
            );
        });
        s = s + 1;
...
fun evaluate_curve_dispatch_equivalence_linear() {
    let shape = curve_shape_policy::new_linear();
    let pairs = pd1_interior_pairs();
    pairs.do_ref!(|p| {
        assert_eq!(
            curve_shape_policy::proj_value(curve_shape_policy::compute_curve_height(&shape, curve_shape_policy::progress(phases::elapsed_for_testing(p.t), phases::duration(p.t_max)))),
...
fun evaluate_curve_dispatch_equivalence_smoothstep() {
    let shape = curve_shape_policy::new_smoothstep();
    let pairs = pd1_interior_pairs();
    pairs.do_ref!(|p| {
        assert_eq!(
            curve_shape_policy::proj_value(curve_shape_policy::compute_curve_height(&shape, curve_shape_policy::progress(phases::elapsed_for_testing(p.t), phases::duration(p.t_max)))),
...
fun evaluate_curve_dispatch_equivalence_logistic() {
    let shape = curve_shape_policy::new_logistic();
    let pairs = pd1_interior_pairs();
    pairs.do_ref!(|p| {
        assert_eq!(
            curve_shape_policy::proj_value(curve_shape_policy::compute_curve_height(&shape, curve_shape_policy::progress(phases::elapsed_for_testing(p.t), phases::duration(p.t_max)))),
...
fun evaluate_curve_dispatch_equivalence_power_law() {
    let shape = curve_shape_policy::new_power_law(2, 1);
    let pairs = pd1_interior_pairs();
    pairs.do_ref!(|p| {
        assert_eq!(
            curve_shape_policy::proj_value(curve_shape_policy::compute_curve_height(&shape, curve_shape_policy::progress(phases::elapsed_for_testing(p.t), phases::duration(p.t_max)))),
...
fun evaluate_curve_dispatch_equivalence_exponential() {
    let shape = curve_shape_policy::new_exponential(2, false);
    let pairs = pd1_interior_pairs();
    pairs.do_ref!(|p| {
        assert_eq!(
            curve_shape_policy::proj_value(curve_shape_policy::compute_curve_height(&shape, curve_shape_policy::progress(phases::elapsed_for_testing(p.t), phases::duration(p.t_max)))),
...
fun eval_linear_golden_vectors() {
    ...
    cases.do_ref!(|case| {
        assert_eq!(curve_shape_policy::eval_linear_for_testing(case.t, case.t_max), case.expected);
    });
}
...
fun eval_linear_midpoint_exact() {
    ...
    let scale = curve_shape_policy::scale_for_testing();
    assert_eq!(curve_shape_policy::eval_linear_for_testing(2, 4), scale / 2);
    assert_eq!(
        curve_shape_policy::eval_linear_for_testing(500_000_000, 1_000_000_000),
        scale / 2,
    );
}
...
fun eval_linear_monotone_in_t() {
    ...
    let len = ts.length();
    while (i < len) {
        let lo = curve_shape_policy::eval_linear_for_testing(ts[i - 1], t_max);
        let hi = curve_shape_policy::eval_linear_for_testing(ts[i],     t_max);
        assert!(lo <= hi, 0);
        i = i + 1;
    };
}
...
fun eval_smoothstep_golden_vectors() {
    ...
    cases.do_ref!(|case| {
        assert_eq!(
            curve_shape_policy::eval_smoothstep_for_testing(case.t, case.t_max),
            case.expected,
        );
    });
}
...
fun eval_smoothstep_midpoint_exact() {
    let scale = curve_shape_policy::scale_for_testing();
    assert_eq!(curve_shape_policy::eval_smoothstep_for_testing(2, 4), scale / 2);
    assert_eq!(
        curve_shape_policy::eval_smoothstep_for_testing(500_000_000, 1_000_000_000),
        scale / 2,
    );
}
...
fun eval_smoothstep_monotone_in_t() {
    ...
    let len = ts.length();
    while (i < len) {
        let lo = curve_shape_policy::eval_smoothstep_for_testing(ts[i - 1], t_max);
        let hi = curve_shape_policy::eval_smoothstep_for_testing(ts[i],     t_max);
        assert!(lo <= hi, 0);
        i = i + 1;
    };
}
...
fun eval_smoothstep_below_linear_first_half_above_second_half() {
    ...
    assert!(
        curve_shape_policy::eval_smoothstep_for_testing(lo_t, t_max)
            < curve_shape_policy::eval_linear_for_testing(lo_t, t_max),
        0,
    );
    // second half: smoothstep > linear
    let hi_t: u64 = 3_000_000_000; // x=0.75
    assert!(
        curve_shape_policy::eval_smoothstep_for_testing(hi_t, t_max)
            > curve_shape_policy::eval_linear_for_testing(hi_t, t_max),
        0,
    );
}
...
fun eval_smoothstep_approximate_symmetry() {
    ...
    while (i < len) {
        let t = probes[i];
        let a = curve_shape_policy::eval_smoothstep_for_testing(t,         t_max);
        let b = curve_shape_policy::eval_smoothstep_for_testing(t_max - t, t_max);
        let sum = a + b;
...
fun eval_power_law_d1_golden_vectors() {
    ...
    cases.do_ref!(|case| {
        assert_eq!(
            curve_shape_policy::eval_power_law_for_testing(case.t, case.t_max, case.n, case.d),
            case.expected,
        );
    });
}
...
fun eval_power_law_monotone_in_t() {
    ...
    while (p < plen) {
        ...
        let tlen = ts.length();
        while (i < tlen) {
            let lo = curve_shape_policy::eval_power_law_for_testing(ts[i - 1], t_max, n, d);
            let hi = curve_shape_policy::eval_power_law_for_testing(ts[i],     t_max, n, d);
            assert!(lo <= hi, 0);
            i = i + 1;
        };
        p = p + 1;
...
fun eval_power_law_range() {
    ...
    while (p < plen) {
        ...
        let tlen = ts.length();
        while (i < tlen) {
            let r = curve_shape_policy::eval_power_law_for_testing(ts[i], t_max, n, d);
            assert!(r <= scale, 0);
            i = i + 1;
        };
        p = p + 1;
...
fun eval_power_law_below_linear_when_convex() {
    ...
    while (p < plen) {
        ...
        while (i < tlen) {
            let r   = curve_shape_policy::eval_power_law_for_testing(ts[i], t_max, n, d);
            let lin = curve_shape_policy::eval_linear_for_testing(ts[i], t_max);
            assert!(r < lin, 0);
            i = i + 1;
        };
        p = p + 1;
...
fun eval_power_law_root_golden_vectors() {
    ...
    assert_eq!(curve_shape_policy::eval_power_law_for_testing(1, 4,  1, 2), 500_000_000);
    // α = 3/2 convex: 0.25^1.5 · SCALE = 1.25·10⁸ exactly
    assert_eq!(curve_shape_policy::eval_power_law_for_testing(1, 4,  3, 2), 125_000_000);
    // α = 1/3 concave: ∛(1/8) · SCALE = 5·10⁸ exactly (perfect cube)
    assert_eq!(curve_shape_policy::eval_power_law_for_testing(1, 8,  1, 3), 500_000_000);
    // α = 1/4 concave: (1/16)^(1/4) · SCALE = 5·10⁸ exactly (perfect 4th power; SCALE_CB branch)
    assert_eq!(curve_shape_policy::eval_power_law_for_testing(1, 16, 1, 4), 500_000_000);
    // α = 1/2 at t = 0.75·t_max: floor(√0.75 · SCALE); √0.75 ≈ 0.866025403784...
    assert_eq!(curve_shape_policy::eval_power_law_for_testing(3, 4,  1, 2), 866_025_403);
    ...
    assert_eq!(curve_shape_policy::eval_power_law_for_testing(2_000_000_000, 4_000_000_000, 1, 2), 707_106_781);
}
...
fun exp_scaled_y_zero_returns_taylor_scale_regardless_of_neg() {
    let ts = curve_shape_policy::taylor_scale_for_testing();
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(0, 1, false), ts);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(0, 1, true),  ts);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(0, 7, false), ts);
    assert_eq!(curve_shape_policy::exp_scaled_pos_for_testing(0, 1),    ts);
    assert_eq!(curve_shape_policy::exp_scaled_pos_for_testing(0, 7),    ts);
}
...
fun exp_scaled_pos_strictly_monotone_over_seed_set() {
    ...
    let len = nums.length();
    while (i < len) {
        let lo = curve_shape_policy::exp_scaled_pos_for_testing(nums[i - 1], dens[i - 1]);
        let hi = curve_shape_policy::exp_scaled_pos_for_testing(nums[i],     dens[i]);
        assert!(lo < hi, 0);
        i = i + 1;
    };
}
...
fun exp_scaled_reciprocal_identity_within_one_ulp() {
    let ts_sq = curve_shape_policy::taylor_scale_sq_for_testing();
    let nums = vector[1u64, 1, 2, 3, 4, 6, 8, 7, 15];
    let dens = vector[2u64, 1, 1, 1, 1, 1, 1, 2,  2];
    let mut i = 0;
    let len = nums.length();
    while (i < len) {
        let pos  = curve_shape_policy::exp_scaled_for_testing(nums[i], dens[i], false);
        let neg  = curve_shape_policy::exp_scaled_for_testing(nums[i], dens[i], true);
        let prod = pos * neg;
        assert!(prod <= ts_sq, 0);
...
fun exp_scaled_pos_y_eq_8_positive_and_within_overflow_budget() {
    let v = curve_shape_policy::exp_scaled_pos_for_testing(8, 1);
    assert!(v > 0, 0);
}
...
fun exp_a_norm_dispatcher_per_pair() {
    ...
    while (a <= 8) {
        assert_eq!(
            curve_shape_policy::exp_a_norm_for_testing(a, false),
            pos_norms[(a - 1) as u64],
        );
        assert_eq!(
            curve_shape_policy::exp_a_norm_for_testing(a, true),
...
fun exp_a_norm_pos_strictly_monotone_in_alpha() {
    let mut a: u8 = 1;
    while (a < 8) {
        let lo = curve_shape_policy::exp_a_norm_for_testing(a,     false);
        let hi = curve_shape_policy::exp_a_norm_for_testing(a + 1, false);
        assert!(lo < hi, 0);
        a = a + 1;
    };
}
...
fun exp_a_norm_neg_strictly_monotone_in_alpha() {
    let mut a: u8 = 1;
    while (a < 8) {
        let lo = curve_shape_policy::exp_a_norm_for_testing(a,     true);
        let hi = curve_shape_policy::exp_a_norm_for_testing(a + 1, true);
        assert!(lo < hi, 0);
        a = a + 1;
    };
}
...
fun eval_exponential_range() {
    ...
    while (p < plen) {
        ...
        let tlen = ts.length();
        while (i < tlen) {
            let r_pos = curve_shape_policy::eval_exponential_for_testing(ts[i], t_max, a, false);
            let r_neg = curve_shape_policy::eval_exponential_for_testing(ts[i], t_max, a, true);
...
fun eval_exponential_monotone_in_t() {
    ...
    while (p < plen) {
        ...
        let tlen = ts.length();
        while (i < tlen) {
            let lo_pos = curve_shape_policy::eval_exponential_for_testing(ts[i - 1], t_max, a, false);
            let hi_pos = curve_shape_policy::eval_exponential_for_testing(ts[i],     t_max, a, false);
            assert!(lo_pos <= hi_pos, 0);
            let lo_neg = curve_shape_policy::eval_exponential_for_testing(ts[i - 1], t_max, a, true);
            let hi_neg = curve_shape_policy::eval_exponential_for_testing(ts[i],     t_max, a, true);
...
fun eval_exponential_below_linear_when_convex() {
    ...
    while (p < plen) {
        ...
        let tlen = ts.length();
        while (i < tlen) {
            let r   = curve_shape_policy::eval_exponential_for_testing(ts[i], t_max, a, false);
            let lin = curve_shape_policy::eval_linear_for_testing(ts[i], t_max);
            assert!(r < lin, 0);
            i = i + 1;
        };
...
fun eval_exponential_above_linear_when_concave() {
    ...
    while (p < plen) {
        ...
        let tlen = ts.length();
        while (i < tlen) {
            let r   = curve_shape_policy::eval_exponential_for_testing(ts[i], t_max, a, true);
            let lin = curve_shape_policy::eval_linear_for_testing(ts[i], t_max);
            assert!(r > lin, 0);
            i = i + 1;
        };
...
fun eval_exponential_complementarity_within_4_ulp() {
    ...
    while (i < len) {
        ...
        let t_max = tmaxs[i];
        let a     = alphas[i];
        let lhs = curve_shape_policy::eval_exponential_for_testing(t,         t_max, a, false);
        let rhs = curve_shape_policy::eval_exponential_for_testing(t_max - t, t_max, a, true);
        let sum = lhs + rhs;
...
fun eval_exponential_golden_vectors() {
    ...
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(1, 4, 2, false), 101_536_324);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(1, 4, 2, true ), 455_054_233);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(1, 4, 4, false),  32_058_603);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(1, 4, 8, false),   2_144_008);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(1, 4, 8, true ), 864_954_876);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(1, 4, 1, true ), 349_932_008);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(2, 4, 2, false), 268_941_421);
    assert_eq!(curve_shape_policy::eval_exponential_for_testing(2, 4, 2, true ), 731_058_578);
}
...
fun eval_logistic_midpoint_exact_scale_half() {
    let scale = curve_shape_policy::scale_for_testing();
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(2_000_000_000, 4_000_000_000), scale / 2);
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(500, 1_000),                   scale / 2);
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(2, 4),                         scale / 2);
}
...
fun eval_logistic_range() {
    ...
    let len = ts.length();
    while (i < len) {
        let r = curve_shape_policy::eval_logistic_for_testing(ts[i], t_max);
        assert!(r <= scale, 0);
        i = i + 1;
    };
}
...
fun eval_logistic_monotone_in_t() {
    ...
    let len = ts.length();
    while (i < len) {
        let lo = curve_shape_policy::eval_logistic_for_testing(ts[i - 1], t_max);
        let hi = curve_shape_policy::eval_logistic_for_testing(ts[i],     t_max);
        assert!(lo <= hi, 0);
        i = i + 1;
    };
}
...
fun eval_logistic_below_linear_first_half_above_second_half() {
    ...
    assert!(
        curve_shape_policy::eval_logistic_for_testing(lo_t, t_max)
            < curve_shape_policy::eval_linear_for_testing(lo_t, t_max),
        0,
    );
    ...
    assert!(
        curve_shape_policy::eval_logistic_for_testing(hi_t, t_max)
            > curve_shape_policy::eval_linear_for_testing(hi_t, t_max),
        0,
    );
}
...
fun eval_logistic_approximate_symmetry_within_2_ulp() {
    ...
    while (i < len) {
        let t     = ts[i];
        let t_max = tmaxs[i];
        let a = curve_shape_policy::eval_logistic_for_testing(t,         t_max);
        let b = curve_shape_policy::eval_logistic_for_testing(t_max - t, t_max);
        let sum = a + b;
...
fun eval_logistic_sigma_floor_denom_algebraic_identity() {
    let scale = curve_shape_policy::scale_for_testing() as u128;
    let denom = curve_shape_policy::logistic_denom_for_testing() as u128;
    let sf    = curve_shape_policy::logistic_sigma_floor_for_testing();
    let sum   = 2 * sf + denom;
...
fun eval_logistic_denom_reference_within_100_ulp() {
    let denom: u64 = curve_shape_policy::logistic_denom_for_testing();
...
fun eval_logistic_golden_vectors() {
    // x = 0.25 — below linear
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(1_000_000_000, 4_000_000_000),  45_176_659);
    // x = 0.75 — above linear; complementary to row above
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(3_000_000_000, 4_000_000_000), 954_823_340);
    ...
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(1, 4),                          45_176_659);
}
...
fun logistic_vs_smoothstep_crossing_at_midpoint() {
    ...
    assert_eq!(
        curve_shape_policy::eval_logistic_for_testing(4_000_000_000, t_max),
        curve_shape_policy::eval_smoothstep_for_testing(4_000_000_000, t_max),
    );
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(4_000_000_000, t_max), scale / 2);
    ...
    while (i < len) {
        let t   = first_half[i];
        let log = curve_shape_policy::eval_logistic_for_testing(t, t_max);
        let ss  = curve_shape_policy::eval_smoothstep_for_testing(t, t_max);
        assert!(log < ss, 0);
        i = i + 1;
    };
...
fun power_law_2_1_below_or_equal_smoothstep_globally() {
    ...
    while (i < len) {
        let t  = ts[i];
        let pl = curve_shape_policy::eval_power_law_for_testing(t, t_max, 2, 1);
        let ss = curve_shape_policy::eval_smoothstep_for_testing(t, t_max);
        assert!(pl <= ss, 0);
        i = i + 1;
    };
...
fun power_law_convex_chain_decreasing_in_alpha() {
    ...
    while (i < len) {
        let t   = ts[i];
        let pl2 = curve_shape_policy::eval_power_law_for_testing(t, t_max, 2, 1);
        let pl3 = curve_shape_policy::eval_power_law_for_testing(t, t_max, 3, 1);
        let pl8 = curve_shape_policy::eval_power_law_for_testing(t, t_max, 8, 1);
        assert!(pl3 < pl2, 0);
        assert!(pl8 < pl3, 0);
        i = i + 1;
    };
...
fun power_law_concave_chain_increasing_in_root_degree() {
    ...
    while (i < len) {
        let t     = ts[i];
        let pl_12 = curve_shape_policy::eval_power_law_for_testing(t, t_max, 1, 2);
        let pl_13 = curve_shape_policy::eval_power_law_for_testing(t, t_max, 1, 3);
        let pl_14 = curve_shape_policy::eval_power_law_for_testing(t, t_max, 1, 4);
...
fun exponential_convex_chain_decreasing_in_alpha() {
    ...
    while (i < len) {
        let t  = ts[i];
        let e2 = curve_shape_policy::eval_exponential_for_testing(t, t_max, 2, false);
        let e4 = curve_shape_policy::eval_exponential_for_testing(t, t_max, 4, false);
        let e8 = curve_shape_policy::eval_exponential_for_testing(t, t_max, 8, false);
        assert!(e4 < e2, 0);
        assert!(e8 < e4, 0);
        i = i + 1;
    };
...
fun exponential_concave_chain_increasing_in_alpha() {
    ...
    while (i < len) {
        let t  = ts[i];
        let e2 = curve_shape_policy::eval_exponential_for_testing(t, t_max, 2, true);
        let e4 = curve_shape_policy::eval_exponential_for_testing(t, t_max, 4, true);
        let e8 = curve_shape_policy::eval_exponential_for_testing(t, t_max, 8, true);
        assert!(e4 > e2, 0);
        assert!(e8 > e4, 0);
        i = i + 1;
    };
...
fun midpoint_convexity_gate() {
    ...
    assert_eq!(curve_shape_policy::eval_linear_for_testing(t, t_max),     half);
    assert_eq!(curve_shape_policy::eval_smoothstep_for_testing(t, t_max), half);
    assert_eq!(curve_shape_policy::eval_logistic_for_testing(t, t_max),   half);
    ...
    while (i < len) {
        let r = curve_shape_policy::eval_power_law_for_testing(t, t_max, convex_ns[i], convex_ds[i]);
        assert!(r < half, 0);
        i = i + 1;
    };
    ...
    while (i < len) {
        let r = curve_shape_policy::eval_power_law_for_testing(t, t_max, concave_ns[i], concave_ds[i]);
        assert!(r > half, 0);
        i = i + 1;
    };
    ...
    let mut a: u8 = 1;
    while (a <= 8) {
        let convex  = curve_shape_policy::eval_exponential_for_testing(t, t_max, a, false);
        let concave = curve_shape_policy::eval_exponential_for_testing(t, t_max, a, true);
...
fun bootstrap_constants_match_pinned() {
    ...
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(1, 1, false), 2_718_281_828_459_045_226);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(1, 1, true ),   367_879_441_171_442_322);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(1, 2, false), 1_648_721_270_700_128_139);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(1, 2, true ),   606_530_659_712_633_426);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(2, 1, false), 7_389_056_098_930_650_216);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(4, 1, false), 54_598_150_033_144_239_050);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(8, 1, false), 2_980_957_986_946_523_322_343);
    assert_eq!(curve_shape_policy::exp_scaled_for_testing(8, 1, true ),   335_462_627_913_225);
    ...
    let mut a: u64 = 1;
    while (a <= 8) {
        let pos_v    = curve_shape_policy::exp_scaled_for_testing(a, 1, false);
        let computed = pos_v - ts;
        assert_eq!(computed, pos_norms[(a - 1) as u64]);
        assert_eq!(curve_shape_policy::exp_a_norm_for_testing(a as u8, false), computed);
        a = a + 1;
    };
    ...
    let mut a: u64 = 1;
    while (a <= 8) {
        let neg_v    = curve_shape_policy::exp_scaled_for_testing(a, 1, true);
        let computed = ts - neg_v;
        assert_eq!(computed, neg_norms[(a - 1) as u64]);
        assert_eq!(curve_shape_policy::exp_a_norm_for_testing(a as u8, true), computed);
        a = a + 1;
    };

    // LOGISTIC_DENOM (§9): ((e⁶·TS − TS) · SCALE) / (e⁶·TS + TS)
    let ek6      = curve_shape_policy::exp_scaled_pos_for_testing(6, 1);
    let computed = (((ek6 - ts) * scale) / (ek6 + ts)) as u64;
    assert_eq!(computed, 995_054_753);
    assert_eq!(curve_shape_policy::logistic_denom_for_testing(), computed);
}
...
fun projectors_linear_variant() {
    let s = curve_shape_policy::new_linear();
...
fun projectors_smoothstep_variant() {
    let s = curve_shape_policy::new_smoothstep();
    assert!(!s.proj_is_linear());
    assert!(s.proj_is_smoothstep());
    assert!(!s.proj_is_logistic());
    assert!(!s.proj_is_power_law());
...
fun projectors_logistic_variant() {
    let s = curve_shape_policy::new_logistic();
    assert!(!s.proj_is_linear());
    assert!(!s.proj_is_smoothstep());
    assert!(s.proj_is_logistic());
    assert!(!s.proj_is_power_law());
...
fun projectors_power_law_variant() {
    let s = curve_shape_policy::new_power_law(3, 2);
    assert!(!s.proj_is_linear());
    assert!(!s.proj_is_smoothstep());
    assert!(!s.proj_is_logistic());
    assert!(s.proj_is_power_law());
...
fun projectors_exponential_variant() {
    let s = curve_shape_policy::new_exponential(5, true);
    assert!(!s.proj_is_linear());
    assert!(!s.proj_is_smoothstep());
    assert!(!s.proj_is_logistic());
    assert!(!s.proj_is_power_law());
