#!/usr/bin/env tsx
/**
 * Phase A / 18 — governance over a fleet: one OwnerCap retires N escrows
 *
 * Tests the core claim of the pure-governance-token model: a single OwnerCap
 ...
 *
 * Measured two ways, both with the SAME &OwnerCap:
...
async function generatePortfolio(
...
): Promise<{ escrows: string[]; ownerCapId: string }> {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (i === 0) {
      ...
      buildIntegrateIntoPortfolio(tx, d.usufructPackageId, d.dummyAssetPackageId, d.protocolFeeRefId, ownerCapId, inboxId);
    }
  ...
  console.log(` ✓  cap=${ownerCapId.slice(0, 10)}…`);
  return { escrows, ownerCapId };
}
...
async function main() {
  ...
  console.log(`governance_fleet_retire — one OwnerCap over N escrows`);
  console.log(`N_max=${N_MAX}  values=[${N_VALUES.join(', ')}]\n`);

  const results: any[] = [];
  for (const n of N_VALUES) {
    console.log(`\n─── N = ${n} ───`);
    const { escrows, ownerCapId } = await generatePortfolio(client, kp, d, 2 * n);
    ...
    for (let i = 0; i < n; i++) {
      ...
      tx.moveCall({ target: `${d.usufructPackageId}::escrow::retire`, typeArguments: TA(d),
        arguments: [tx.object(sep[i]), tx.object(ownerCapId), clock(tx)] });
      const rec = await measure(client, kp.owner, `retire_sep_${n}_${i}`, i, tx);
    ...
    txB.setSender(d.owner.address);
    for (let i = 0; i < n; i++) {
      txB.moveCall({ target: `${d.usufructPackageId}::escrow::retire`, typeArguments: TA(d),
        arguments: [txB.object(batch[i]), txB.object(ownerCapId), clock(txB)] });
    }
    const recB = await measure(client, kp.owner, `retire_batch_${n}`, 0, txB);
