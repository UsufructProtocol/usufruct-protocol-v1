#!/usr/bin/env tsx
/**
 * Phase A / 14 — update_usufructuary_refund_address
 * Measures: usufructuary redirects the refund destination on the active seat.
 *
 * Precondition: escrow Occupied with usufructuary1 holding the UsufructCap.
 *   1. Integrate (minimal ensemble, 60s tenure)
 *   2. usufructuary1 rents → Occupied
 *   Now the cap holder can call update_usufructuary_refund_address with a
 ...
 *   - escrow::update_usufructuary_refund_address(escrow, &cap, refund, clock)
 *
 * Authority is the cap reference; the call's sender (usufructuary1) is not
...
async function setupOccupied(
  ...
  // 2. usufructuary1 rents → Occupied, cap held by usufructuary1
  ...
  return { escrowId: escrowObj.objectId, usufructCapId: capObj.objectId };
}
...
async function main() {
  ...
  for (let run = 0; run < RUNS; run++) {
    ...
    const { escrowId, usufructCapId } = await setupOccupied(client, kp.governor, kp.usufructuary1, d);
    ...
    tx.moveCall({
      ...
      arguments:     [tx.object(escrowId), tx.object(usufructCapId), refund, clock(tx)],
    });

    const rec = await measure(client, kp.usufructuary1, 'update_usufructuary_refund_address', run, tx);
    records.push(rec);
  ...
  saveRecords(resolve(DIR, '../results/a_14_update_usufructuary_refund_address.json'), records);
}
