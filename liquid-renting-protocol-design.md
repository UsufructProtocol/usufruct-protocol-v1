...
### Incentive Alignment with the Integrator

The revenue model binds the protocol's economic outcome to the integrating protocol's in two structural ways, without any explicit coordination or shared governance.

**Value generation is a precondition for value capture.** Protocol fees flow only when `used_credit` flows — when a tenant's stake has been consumed as rent actually earned. There is no listing fee at integration, no subscription during `Idle`, no tax on locked stake, no fee on takeovers that are superseded before expiry. If the integrator's asset attracts no rental demand, the protocol earns nothing regardless of how long the escrow remains active. The protocol's revenue curve is a subset of the integrator's: both parties earn only when the asset's usus is genuinely demanded by the market, and both earn zero during the same periods.

**Revenue is denominated in the integrator's chosen currency.** The generic `RentalEscrow<Asset, CoinType>` parameter leaves the integrator in full control of the payment currency. Protocol fees accumulate in that same `CoinType` — the protocol does not demand payment in SUI, in a governance token, or in any fixed unit of account. If the integrator selected a stablecoin because their asset is denominated in stable units, the protocol earns in that stablecoin; if the integrator selected their own native token, the protocol earns in that token and holds its price exposure identically to the integrator. The protocol cannot extract a share denominated in a different asset while the integrator bears the volatility alone — they share the same unit of account by construction.

Together, these properties make the protocol a co-participant in each integration rather than a rent-taker priced in external terms. The integrator cannot arrive at a state where the protocol profits while their own economics deteriorate — in timing, in volume, or in currency, the protocol's revenue moves only when and as the integrator's does. The basket heterogeneity that motivates the 10% nominal rate (see below) is the direct consequence of this alignment: the protocol chose to bear the same price exposure as each of its integrators rather than price its fee above them in a reserve currency.

### Protocol Fee Rate

The protocol fee rate is **10%**, hardcoded at the module level and identical across all integration instances. It is not configurable by integrators and not subject to per-instance variation. Of every unit of `used_credit`, the owner receives 90% and the protocol retains 10%.

**What the fee compensates.** The rate of any DeFi protocol fee is determined by four structural axes: the protocol's passivity (does it run deterministically or require continuous human labor), the risk it absorbs (systemic failure modes borne by the protocol itself), the ongoing operational cost it must fund (indexers, audits, keepers, infrastructure), and the counterfactual available to integrators (alternatives, substitutes, replicability). Placing Liquid Renting against these axes:

- **Passivity:** total. Lazy evaluation and bijectivity eliminate keepers, strategists, and mid-flight decisions. The state machine runs on transaction-time alone.
- **Risk absorption:** zero systemic risk. No liquidations, no oracles, no bad debt, no stale state. The protocol cannot fail economically — the mathematical structure forecloses it.
- **Operational cost:** real but bounded — the off-chain indexer, SDK, and integrator support required by the event layer. The ambitious star schema documented in the rental_escrow specification is load-bearing for analytics, and maintaining it is an ongoing cost the fee must finance.
- **Counterfactual:** the protocol is novel and without direct substitutes today, but the specification is public and the mathematics are not patentable. Monopoly pricing invites forks; the rate must be defensible, not extractive.

These axes place Liquid Renting in the passive yield share bucket — structurally adjacent to Lido (10% on staking rewards), Aave reserve factors on stable markets (10–15%), and EigenLayer. Protocols that charge meaningfully above this band — Yearn at 20%, Aave on volatile markets at 25% — compensate for attributes Liquid Renting does not carry: active strategy management or explicit absorption of systemic risk. Being differentiated within a bucket does not move a protocol out of that bucket; it positions it at the bucket's upper edge.

Within the passive yield share band, Liquid Renting sits at that upper edge. It provides more than any single comparable: composability via the recursive OwnerCap-in-escrow pattern (§5), a complete market layer rather than a single-yield primitive, and the star schema that makes every flow of value analytically traceable without envelope-metadata dependence. These are within-bucket differentiators — they justify the top of the band, not a move to a bucket the protocol does not structurally belong to.

**The basket haircut.** Because `RentalEscrow<Asset, CoinType>` is generic over `CoinType`, protocol fees accumulate in whatever currency each integration selected. The `ProtocolFeeInbox` therefore aggregates a heterogeneous basket — liquid stablecoins and SUI alongside long-tail integrator tokens of variable or near-zero market value. The nominal fee rate overstates the realized fee: a headline rate that looks reasonable under the assumption of a clean basket delivers less once illiquid positions are marked down or discarded. A 10% nominal rate absorbs this structural discount while remaining within the passive yield share bucket; a lower rate does not.
...
This immutability is asymmetric in its error direction. A rate set too high can be lowered in a subsequent module version with no structural consequence — new integrations adopt the new rate, existing instances continue under the one they entered with, and the protocol remains credible. A rate set too low cannot be raised for the existing cohort: those integrations are permanently anchored at the undervalued rate, and no governance path corrects them. The correct direction of uncertainty is therefore upward. 10% reflects this: it is the bucket-appropriate rate, not the minimum viable one.
